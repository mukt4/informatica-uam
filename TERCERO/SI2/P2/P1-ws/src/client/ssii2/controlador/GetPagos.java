...
public class GetPagos extends ServletRaiz {
    ...
    */
    ...
    //Modificaciones Ejercicio 9
    BindingProvider bp = (BindingProvider) dao;
    bp.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, getServletContext().getInitParameter("VisaDAOWService"));

    if(service == null || dao == null){
      enviaError(new Exception("Stub incorrecto.Get Pago."), request, response);
      return;
    }
    ...
    List<PagoBean> lst = dao.getPagos(idComercio);
    PagoBean[] pagos = new PagoBean[lst.size()];
    pagos = lst.toArray(pagos);
    //List<PagoBean> pagos = Arrays.asList(dao.getPagos(idComercio).toArray());
