...
private void printAddresses(HttpServletRequest request, HttpServletResponse response) throws SocketException {
    ...
    */
    ...
    if(service == null || dao == null){
      enviaError(new Exception("Stub incorrecto.Procesa Pago."), request, response);
      return;
    }
        ...
        PagoBean ret = dao.realizaPago(pago);
	       if (ret == null) {
            enviaError(new Exception("Pago incorrecto"), request, response);
            return;
        }

        request.setAttribute(ComienzaPago.ATTR_PAGO, ret);
