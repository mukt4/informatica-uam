...
public class GetPagos extends ServletRaiz {
    ...
    */
    ...
    /*El metodo getPagos devuelve un ArrayList<PagoBean> por lo que hay que convertirlo a un array de PagoBean*/
    PagoBean[] pagos = new PagoBean[dao.getPagos(idComercio).size()];
    for(i=0; i < dao.getPagos(idComercio).size(); i++){
      pagos[i] = dao.getPagos(idComercio).get(i);
    }
