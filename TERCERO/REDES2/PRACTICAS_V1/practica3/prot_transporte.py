...
class Prot_transporte:
	...
	def __init__(self, app, nick, port, cap):
		...
		self.sockControl = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		self.sockControl.bind(('', self.port-1))
		self.sockControl.listen(10)
		...
		while True:
			...
			if dataParsed[0] != 'CALLING':
				...
				if dataParsed[0] == 'EXIT':
					sys.exit()
				# En caso de que el otro usuario quiera iniciar un chat a traves de mensajes
				elif dataParsed[0] == 'MESSAGE':
					self.sem.acquire()
					if(self.llamadaEstablecida == 1):
						# Si nos encontramos en una llamada o estamos en un chat rechazamos
						self.sem.release()
						sock.send(('MESSAGE_DENIED ' + self.nick).encode()) 
						sock.close()
					# Si no nos encontramos en llamada inciamos chat de texto
					else:
						self.llamadaEstablecida = 1
						self.sem.release()
						answer = self.app.yesNoBox('Chat de texto entrante', '¿Quieres aceptar el chat de ' + dataParsed[1] + '?')
						# Aceptmos la llamada entrante
						if answer == True:
							# Se usara el mismo puerto que en el caso de las llamadas, ya que ambas situaciones no se pueden dar a la vez
							sock.send(('MESSAGE_ACCEPTED ' + self.nick + ' ' + str(self.port)).encode())
							# Creamos el hilo que va a gestionar la llamada, los sockets se cierran ahi
							hiloGestionLlamadas = threading.Thread(target=self.atencionChat, args=(sock, addr, dataParsed))
	...
	def comenzarLlamada(self, ip, puerto, nick):
		...
		self.sockControlDestino.send(('CALLING ' + self.nick + ' ' + str(self.port)).encode())
		...
		if parsedMensajeRecibido[0] == 'CALL_ACCEPTED':
			...
			self.sockEnviar = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
			# Cogenmos la direccion Ip de la conexion TCP y el puerto que nos indica la otra persona
			direccionDestino = (ip, int(parsedMensajeRecibido[2]) + 1)
	...
	def enviarComandosControl(self, sockControlDestino):
		...
		while True:
			...
			if estadoVideoAnterior != self.estadoVideo: # El estado del video anterior es distinto que el estado del video actual (se ha hecho alguna accion)
				...
				elif estadoVideoAnterior == 2:
					# Se ha cortado la llamada, lo notificamos asi al otro lado y cerramos el hilo
					sockControlDestino.send(('CALL_END').encode())
					sys.exit()
				else:
					# Estamos en un chat y se pide la finalizacion del mismo
					sockControlDestino.send(('MESSAGE_END').encode())
					sys.exit()
			else:
	...
	def recibirComandosControl(self, sockControlDestino):
		while True:
			...
			dataReaded = dataReaded.decode()
			# Le asignamos el valor recibido con su valor correspondiente
			if dataReaded == 'CALL_RESUME':
				valueReaded = 0
			elif dataReaded == 'CALL_HOLD':
				valueReaded = 1
			elif dataReaded == 'CALL_END':
				valueReaded = 2
			elif dataReaded == 'MESSAGE_END':
				valueReaded = 3
			else:
				valueReaded = -1
			...
			# Si value readed es 3 significara que se ha finalizado el chat
			if valueReaded == 2 or valueReaded == 3:
				sys.exit()
	...
	# MESSAGE_END
	# Por desarrollar
	def atencionChat(self, sock, addr, dataParsed):
		# Abrir socket UDP dataParsed[2] de envio
		self.sockEnviar = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		# Cogenmos la direccion Ip de la conexion TCP y el puerto que nos indica la otra persona
		direccionDestino = (addr[0], int(dataParsed[2]))
		...
		# Se crean los dos hilos de envio y recibo de datos (UDP)
		hiloEnviar = threading.Thread(target=self.enviarMensaje, args=(self.sockEnviar, direccionDestino))
		hiloRecibir = threading.Thread(target=self.recibirMensaje, args=(self.sockRecibir,))
		hiloEnviar.start()
		hiloRecibir.start()
	...
	"""
	NAME: comenzarChat
	DEFINITION: Funcion encargada de comenzar un chat con otro usuario
	PARAMETERS: ip - Ip del otro extremo del chat
				puerto - Puerto de escucha del otro extremo de la llamada
				nick - Nick del usuario al que se esta llamando
	RETURN: void
	"""
	def comenzarChat(self, ip, puerto, nick):
		self.sem.acquire()
		if self.llamadaEstablecida == 1:
			self.sem.release()
			#Error mostrado en caso de estar en una llamada
		...
		# Envio CALLING y el puerto UDP de escucha
		self.sockControlDestino.send(('MESSAGE ' + self.nick + ' ' + str(self.port)).encode())
		# Se recibe la respuesta de la otra persona
		mensajeRecibido = self.sockControlDestino.recv(1024)
		# Se parsea la llamada
		parsedMensajeRecibido = mensajeRecibido.decode().split()
		#Si el cliente ha aceptado la llamada
		if parsedMensajeRecibido[0] == 'MESSAGE_ACCEPTED':
			# Abrir socket UDP dataParsed[2] de envio
			self.sockEnviar = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
			# Cogenmos la direccion Ip de la conexion TCP y el puerto que nos indica la otra persona
			direccionDestino = (ip, int(parsedMensajeRecibido[2]) + 1)

			# Creamos el hilo de envio de los datos de control
			hiloEnvioControl = threading.Thread(target=self.enviarComandosControl, args=(self.sockControlDestino,))
			hiloEnvioControl.start()
	...
	def enviarMensaje(self, sockEnviar, direccionDestino):
		# 0 significa estamos en una llamada
		# 1 significa que esta pausado
		# Otro valor que hemos colgado
		while True:
			self.semVideo.acquire()
			#En caso de que el video no este en proceso de colgarse
			if self.estadoVideo != 3:
				self.semVideo.release()
				cadena = input("Escribe el mensaje que deseas enviar: ")
				sockEnviar.sendto(cadena.encode(), direccionDestino)
			# Si el video esta en proceso de colgarse, se cierra el socket y se termina el hilo
			elif self.estadoVideo == 3:
				self.semVideo.release()
				# Cerramos el socket
				sockEnviar.close()
				# Se cierra el hilo
	...
	def recibirMensaje(self, sockRecibir):
		# 0 significa estamos en una llamada
		# 1 significa que esta pausado
		# Otro valor que hemos colgado
		while True:
			...
			if self.estadoVideo != 3:
				self.semVideo.release()
				dataReaded = sockRecibir.recv(1024)
				print("Mensaje recibido: " + dataReaded.decode())
			elif self.estadoVideo == 3:
				self.semVideo.release()
				# Este socket no hay que cerrarlo
				# Se cierra el hilo
